...
static bool toggle_opt_menu_item( HWND hWnd, UINT uMenuId, const TCHAR * pIniOptionName, int nYesValueInINI = 1 );
...
WriteDebugLog(s);
			...
			// When enabled "ListAll" is not 1 but 2 in INI, for historical reason (2014-04-05 v1.7.0.21)
			...
			g_bCAIO = (BOOL) CheckMenuItemI( hMenu, IDM_CAIO, _T("CAIO"), strIniPath );
			...
			SIZE size;
			const int cchURL = (int) _tcslen( APP_HOME_URL );
			GetTextExtentPoint32( hDC, APP_HOME_URL, cchURL, &size );
...
///				SetThreadPriority( hChildThread[ iMyId ], THREAD_PRIORITY_TIME_CRITICAL );
		...
		case WM_USER_STOP:
		{
			...
			// wParam may be JUST_UPDATE_STATUS (0xDA00), when iMyId>MAX_SLOTS and no target will be unlimited,
			// but i/f status (e.g. menu item enabled/grayed-out) will be updated.
			unsigned iMyId = LOWORD( wParam );
			...
			if( IsActive() )
			{
				...
				{
					wcscpy_s( s_szLimitingStatus, _countof(s_szLimitingStatus), S_RUS_2000 );
				}
				else if( IS_SPANISH )
				{
...
#define WM_XBUTTONDOWN                  0x020B
		...
		case WM_COMMAND:
		{
			...
			switch( id )
			{
				...
				case IDM_ALLOWMULTI:
					toggle_opt_menu_item( hWnd, id, _T("AllowMulti") );
					break;
				...
				case IDM_LOWER_PRIVILEGE:
					toggle_opt_menu_item( hWnd, id, _T("LowerPriv") );
					break;
				...
				case IDM_DISABLE_F1:
					toggle_opt_menu_item( hWnd, id, _T("DisableF1") );
					break;

				case IDM_CAIO:
					g_bCAIO = (BOOL) toggle_opt_menu_item( hWnd, id, _T("CAIO") );
					break;
				...
				case IDM_HOMEPAGE:
					if( MessageBox( hWnd, APP_HOME_URL, _T("Try to Open?"), MB_OKCANCEL | MB_ICONQUESTION ) == IDOK )
...
#define UInt32_(l) ((UINT32)(l))
		...
		case WM_PAINT:
		{
			...
			if( _tcscmp( s_szStatus[ 0 ], APP_LONGNAME ) == 0 )
			{
				...
				{
					WCHAR str[ cchStatus ] = L"";
					wcscpy_s( str, cchStatus, S_RUS_0001 );
					_tcscat_s( s_szStatus[ 2 ], cchStatus, str );
				}
				else if( IS_SPANISH )
				{
			...
			const int cchURL = (int) _tcslen( APP_HOME_URL );
			TextOut( hdc, urlrect.left, urlrect.top, APP_HOME_URL, cchURL );
			SIZE size;
			GetTextExtentPoint32( hdc, APP_HOME_URL, cchURL, &size );
...
static bool toggle_opt_menu_item( HWND hWnd, UINT uMenuId, const TCHAR * pIniOptionName, int nYesValueInINI )
{
	HMENU hMenu = GetMenu( hWnd );
	...
	TCHAR szValue[ 10 ];
	_stprintf_s( szValue, _countof(szValue), _T("%d"), fEnable ? nYesValueInINI : 0 );
	WritePrivateProfileString(
		TEXT( "Options" ), 
		pIniOptionName,
		szValue,
