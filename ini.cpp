...
// GetSystemDefaultLangID = the language used when displaying text in programs that do not support Unicode
// GetUserDefaultLangID = "format" in Control Panel / Clock, Language, and Region
// GetUserDefaultUILanguage
//0x04 	LANG_CHINESE
//0x01 	SUBLANG_CHINESE_TRADITIONAL 	Chinese (Traditional)
//0x02 	SUBLANG_CHINESE_SIMPLIFIED 	Chinese (Simplified)

	const LANGID wLangId = GetUserDefaultUILanguage();
	WORD wDefLanguage = (WORD) PRIMARYLANGID( wLangId );

	if( wDefLanguage == LANG_CHINESE )
	{
		wDefLanguage = ( SUBLANGID( wLangId ) == SUBLANG_CHINESE_TRADITIONAL ) ? LANG_CHINESE_T : LANG_CHINESE_S ;
	}
	else if( wDefLanguage != LANG_ENGLISH && wDefLanguage != LANG_FINNISH && wDefLanguage != LANG_JAPANESE
		&& wDefLanguage != LANG_SPANISH && wDefLanguage != LANG_RUSSIAN )
	{
	...
	if( wLanguage != LANG_ENGLISH && wLanguage != LANG_FINNISH && wLanguage != LANG_JAPANESE && wLanguage != LANG_JAPANESEo
		...
		&& wLanguage != LANG_FRENCH && wLanguage != LANG_RUSSIAN )
	{
...
BOOL UpdateIFF_Ini( BOOL bCleanSliderSection )
{
	...
	// 2021-07-06 FIX: Do not delete, if AbsFoe (Hardcoded "enemy")
	...
	if( 0 < dwcch && dwcch < (DWORD) cchBuffer - 2 )
	{
		ptr = lpBuffer;
		while( *ptr ) {
			if( ReadIni_Time( ptr ) == (UINT64) 0 && _tcscmp( ptr, _T("Slider2") ) != 0 && ! IsAbsFoe( ptr ) ) {
