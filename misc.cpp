...
INT_PTR CALLBACK About( HWND hDlg, UINT uMessage, WPARAM wParam, LPARAM lParam )
{
	...
	static RECT urlrect;// = { 90, 140, 500, 170 };
...
#define CPU_ TEXT(" No-SSE2")
			...
			{
				wcscpy_s( lpWindowText, MAX_WINDOWTEXT, S_RUS_9000 );
				SetWindowText( hDlg, lpWindowText );
				SetDlgItemText( hDlg, IDC_DESCRIPTION, S_RUS_DESC );
				SetDlgItemText( hDlg, IDC_EDITBOX_GPL, TEXT_GPL );
			}
			else if( IS_SPANISH )
			{
...
void ShowCommandLineHelp( HWND hWnd )
{
	...
	MessageBox( hWnd,
				...
				_T("--exitnow, -e, -x\r\n\t= Exit now.\r\n")
				_T("--allow-multi\r\n\t= Allow multiple instances of BES; not usable with -J or -a.\r\n")
			...
			//	_T("--watch-multi\r\n\t= Watch multiple instances of the target EXE.\r\n")
				...
				_T("\r\n")
				_T("You can use a wildcard, e.g. \"path\\folder\\*.exe\"\r\n")
				_T("You can use \"PID:xxxx\" instead of \"path\\to\\target.exe\"\r\n") // 2017-03-17 (1.7.5)
				_T("The target should be quoted with \" and \", including \"PID:xxxx\"\r\n") // 2022-10-28
				,
				APP_NAME, uType );
}
...
static BOOL GshowTest( LPCTSTR tested, LPCTSTR wanted )
{
	if( _tcsicmp( tested, wanted ) == 0 ) return TRUE;

	const TCHAR * pBkSlash = _tcsrchr( tested, _T('\\') );

	if( ! pBkSlash ) return FALSE;

	return ( _tcsncicmp( pBkSlash + 1, wanted, _tcslen( wanted ) ) == 0 );
}
...
)
{
	...
	{
		if( GshowTest( lpText, lpParams->lpTarget ) )
		{
...
BOOL CheckLanguageMenuRadio( const HWND hWnd )
{
	...
	HMENU hOptionMenu = GetSubMenu( hMenu, 2 );
	HMENU hLangMenu = GetSubMenu( hOptionMenu, 4 );
	if( GetLanguage() == LANG_RUSSIAN )
	{
		return CheckMenuRadioItem( hLangMenu, 0, 15, 11, MF_BYPOSITION );
	}

	// check the 1st item tentatively to uncheck "Russian" if it's currently checked
	CheckMenuRadioItem( hLangMenu, 0, 15, 0, MF_BYPOSITION );

	return CheckMenuRadioItem( hMenu, IDM_CHINESE_S, IDM_JAPANESEo,
