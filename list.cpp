...
INT_PTR CALLBACK xList( HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam )
{
	...
	switch( message )
	{
		case WM_INITDIALOG:
		{
			...
			{
				wcscpy_s( szWindowText, MAX_WINDOWTEXT, S_RUS_1000 );
			}
			else if( IS_SPANISH )
			{
				MultiByteToWideChar( CP_UTF8, MB_CUTE,
...
static INT_PTR CALLBACK Question( HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam )
{
	...
	switch ( message )
	{
		case WM_INITDIALOG:
		{
			...
			{
				wcscpy_s( szWindowText, MAX_WINDOWTEXT, S_RUS_1001 );
				wcscpy_s( msg1, 1023, lpTarget->fWatch ? S_QUESTION2_RUS : S_QUESTION1_RUS );
				wcscpy_s( format, _countof(format), S_FORMAT2_RUS );
			}
			else if( IS_SPANISH )
			{
				MultiByteToWideChar( CP_UTF8, MB_CUTE,
...
//				if( PathEqual( arPathInfo[ n ], lpPath, cch, true ) ) ret = n;
				if( PathEqual( arPathInfo[ n ], lpPath, cch, false ) ) ret = n; //2021-11-13
			}
...
{
	// Do nothing if it's not starting with "pid:" (MOD: case-insensitive @ 2022-10-28 [to be back-ported to v1.7.9])
	if( _tcsncicmp( strTarget, _T("pid:"), 4 ) != 0 ) return 0UL;
