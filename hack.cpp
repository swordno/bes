...
extern BOOL g_bCAIO; // if TRUE, don't limit the target when it's foreground
...
{
	PID_AND_HWND * p = (PID_AND_HWND *) lParam;
    DWORD dwPID;
    GetWindowThreadProcessId( hwnd, &dwPID );
    if( dwPID == p->pid && GetWindow( hwnd, GW_OWNER ) == NULL && IsWindowVisible( hwnd ) )
    {
        ((PID_AND_HWND *) lParam)->hwnd = hwnd;
        return FALSE;
    }
    return TRUE;
}
...
static int Hack_Worker( void * pv )
{
	...
	int mode = ti.mode;
	if( mode != 0 && mode != 1 ) mode = DEF_MODE ;
...
//#ifdef CAIO
	PID_AND_HWND p_h;
	p_h.hwnd = NULL;
	p_h.pid = dwTargetProcessId;
	EnumWindows( &EnumProc, (LPARAM) &p_h );
	HWND hwndTarget = p_h.hwnd;
//#endif
		...
		if( iMyId < 3 )
		{
			if( ti.wDelay ) {
				_stprintf_s( ppszStatus[ 0 + iMyId * 4 ], cchStatus,
						_T( "Target #%d [ Limiting in %.1f second(s) ]" ),
						iMyId + 1,
						ti.wDelay / 1000.0 );
			} else {
				_stprintf_s( ppszStatus[ 0 + iMyId * 4 ], cchStatus,
...
//		const bool fActive3 = ( g_bHack[ 2 ] == TRUE && g_dwTargetProcessId[ 2 ] != WATCHING_IDLE );
		...
		//while( msElapsed < 4 * 1024 ) // about 4 seconds
		{
			...
			if( g_bCAIO )
			{
				TCHAR wintext[ 256 ] = _T("");
				GetWindowText( GetForegroundWindow(), wintext, 255 );
				_stprintf_s( msg, _countof(msg), _T("CAIO: %d: text=%s"),
					( GetForegroundWindow() == hwndTarget ? 1 : 0 ),
					wintext );
				WriteDebugLog( msg );
			}
#endif
...
//#endif
			...
			if( g_bCAIO && hwndTarget && GetForegroundWindow() == hwndTarget )
			{
				; // don't sleep
			}
			else
			...
			if( g_bCAIO && hwndTarget && GetForegroundWindow() == hwndTarget )
				Sleep( TimeRed ); // just "waste" the red-light time without limiting
